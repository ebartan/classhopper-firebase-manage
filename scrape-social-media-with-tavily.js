...
async function scrapeSocialMediaWithTavily() {
  try {
    console.log('🔍 Firebase\'den fakülte sosyal medya linkleri alınıyor...\n');

    // Faculty koleksiyonundan sosyal medyası olan fakülteleri çek
    ...
    for (const doc of facultiesSnapshot.docs) {
      ...
      console.log(`\n${'='.repeat(60)}`);
      console.log(`📚 Fakülte: ${facultyData.name}`);
      console.log(`🆔 ID: ${facultyId}`);
      console.log('='.repeat(60));

      // Sosyal medya linklerini kontrol et
      if (!facultyData.socialMedia || Object.keys(facultyData.socialMedia).length === 0) {
        console.log('⚠️  Sosyal medya linki yok, atlanıyor...');
        continue;
      }
      ...
      for (const [platform, config] of Object.entries(platforms)) {
        const platformUrl = facultyData.socialMedia[platform];
        ...
        if (platformUrl && platformUrl.trim() !== '') {
          ...
          const searchQuery = `${facultyData.name} YTU ${platform} ${platformUrl} university faculty ${config.query}`;
    ...
    const outputFile = 'faculty-social-media-tavily-results.json';
    fs.writeFileSync(
      outputFile,
    ...
    console.log('='.repeat(60));
    console.log(`✅ İşlenen fakülte: ${processedCount}`);
    console.log(`🔗 Sosyal medyası olan: ${withSocialMedia}`);
    console.log(`📊 Toplam fakülte: ${facultiesSnapshot.size}`);
